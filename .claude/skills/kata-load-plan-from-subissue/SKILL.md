...
description: "Canonical comparison (after stripping not-flushed blocks on both sides). Materialization of the provider-specific local cache (.claude/plans/plan-{M}-{slug}.md or .cursor/plans/plan-{M}-{slug}.md) from the canonical Plan sub-issue body, per the hierarchical model of lex-agent-planning"
---
